...
- For Linux (or WSL2 users): Run the following to install necessary system packages:
```bash
sudo apt update
sudo apt install build-essential pkg-config libssl-dev libsasl2-dev -y
```
- If you're on Windows, you must use WSL2 and install a Linux distribution like Ubuntu 24.04.
...
1. We'll start by creating a python virtual environment to manage our python install and dependencies. Here's how you can do that with `uv`:
    ...
    ```
    If requirements.txt is missing, it can be skipped. It's only required for the ML pipeline.
    ```bash
